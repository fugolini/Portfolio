...
class Mailer:
    ...
    def send_email(self, email: str) -> None:
        ...
        try:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                ...
                server.send_message(
                    email, from_addr=self.sender, to_addrs=self.recipients
                )
            self.log.info("Email sent.")
        except Exception as e:
            self.log.error(e)

    def _fetch_mailing_data(self) -> tuple[str, list[str]]:
    ...
    def _setup_email(self, msg_to_send: str, error_flag=False) -> EmailMessage:
        ...
        if not error_flag:  # Builds the standard email
            email["Subject"] = (
                f"I giornali di oggi in esclusiva per i non abbonati, i mangiaufo e i parassiti sociali"
            )
    ...
    def _fetch_editions(self, paper_name: str, archive: str) -> tuple[str, str]:
        ...
        latest_edition_date, latest_edition_link = next(
            iter(complete_archive[0].items())
        )
