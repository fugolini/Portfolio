...
KEY_FILE = PROJECT_ROOT / "system" / "key.key"
ADDRESS_BOOK = PROJECT_ROOT / "system" / "address_book.json"


def italian_date(which_day=0, formatter=" ") -> str:
...
def encrypt_credentials(credentials: dict, file_path: str) -> None:
    """Encrypt n credentials (passed as a dictionary in the form
    credential: content) and store them in a given file
    """
...
def decrypt_credentials(file_path: str) -> dict:
...
def print_address_book() -> None:
    """Print the address book"""
    print("\n*** Recipients ***\n")
    with open(ADDRESS_BOOK, "r") as a:
